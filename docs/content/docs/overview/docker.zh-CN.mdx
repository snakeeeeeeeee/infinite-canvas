...
仓库内的 `docker-compose.local.yml` 默认将 SuperToken 授权/new-api 主站设为 `https://supertoken.cc`。需要连接其他授权服务时可在启动命令前覆盖：

```bash
SUPERTOKEN_AUTH_BASE_URL=https://auth.example.com docker compose -f docker-compose.local.yml up -d --build --force-recreate
```

## SuperToken 地址

授权主站和媒体 API 节点使用独立变量。`SUPERTOKEN_AUTH_BASE_URL` 只用于授权页面和授权码兑换；媒体请求默认继续根据渠道区域使用大陆加速 `https://hk.supertoken.cc` 或国际直连 `https://api.supertoken.cc`：
...
只有需要把所有媒体请求统一转发到自建网关时，才额外设置 `SUPERTOKEN_BASE_URL`。修改任一变量后重新创建容器即可生效，无需重新构建 Vite 静态文件。
