...
仓库内的 `docker-compose.local.yml` 默认将 SuperToken/new-api 地址设为 `https://supertoken.cc`。需要连接其他部署时可在启动命令前覆盖：

```bash
SUPERTOKEN_BASE_URL=https://api.example.com docker compose -f docker-compose.local.yml up -d --build --force-recreate
```

## SuperToken 地址

主应用镜像支持在容器启动时通过 `SUPERTOKEN_BASE_URL` 指定 SuperToken/new-api 公网地址，无需为不同环境重新构建 Vite 静态文件：
...
修改后重新创建容器即可生效。未设置时继续根据渠道区域使用内置的大陆加速或国际站点地址。
