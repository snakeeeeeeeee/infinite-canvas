...
- 多图图片节点：批量生成仍只保存为一个图片节点，请求开始前会在 `images` 中创建全部图片槽位，每个槽位通过 `status/errorDetails` 独立记录实时状态，并通过 `asyncTaskId/asyncOriginNodeId` 与 `generationProgress/generationProgressKnown` 保存可恢复任务和真实进度；原生多图任务可由多个槽位共享同一个任务 ID，拆分请求则各自保存任务 ID。最先成功的槽位会立即写入 `primaryImageId`，并由节点级 `content` 和图片元数据展示；展开状态仅属于当前界面，不写入项目数据。
