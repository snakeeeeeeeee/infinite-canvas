...
# SuperToken URLs are Base64-encoded before interpolation so arbitrary URL characters cannot break config.js.
...
SUPERTOKEN_AUTH_BASE_URL_BASE64=$(printf '%s' "${SUPERTOKEN_AUTH_BASE_URL:-}" | base64 | tr -d '\n')
...
window.__RUNTIME_CONFIG__ = {
  ...
  SUPERTOKEN_AUTH_BASE_URL: atob("${SUPERTOKEN_AUTH_BASE_URL_BASE64}")
};
EOF
