...
type ImageReadOptions = { signal?: AbortSignal; storageKey?: string };

export function uploadImage(input: Blob, options?: ImageReadOptions): Promise<UploadedImage & { storageKey: string }>;
export function uploadImage(input: string | Blob, options?: ImageReadOptions): Promise<UploadedImage>;
...
async function storeImage(blob: Blob, options?: ImageReadOptions): Promise<UploadedImage> {
    const storageKey = options?.storageKey || `image:${nanoid()}`;
...
export async function storeGeneratedImage(image: { dataUrl: string; storageKey?: string; width?: number; height?: number; bytes?: number; mimeType?: string }): Promise<UploadedImage> {
    ...
    if (!blob) return uploadImage(image.dataUrl, { storageKey: image.storageKey });
