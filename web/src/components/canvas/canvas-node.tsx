...
function LoadingContent({ node, theme }: Pick<NodeContentRendererProps, "node" | "theme">) {
    ...
    return (
        <GenerationProgress
            compact
            progress={node.metadata?.generationProgress}
            progressKnown={node.metadata?.generationProgressKnown}
            label={t("canvas.node.generating")}
            accentColor={theme.node.activeStroke}
            surfaceColor={theme.node.fill}
...
function ImageSlotStatus({ image }: { image?: CanvasNodeImage }) {
    ...
    return (
        <GenerationProgress
            compact
            progress={image?.generationProgress}
            progressKnown={image?.generationProgressKnown}
            label={t("canvas.node.generating")}
            accentColor={theme.node.activeStroke}
            surfaceColor={theme.node.fill}
            mutedColor={theme.node.muted}
