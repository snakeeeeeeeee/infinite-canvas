...
export function resetInterruptedGeneration(nodes: CanvasNodeData[]) {
    ...
    return nodes.map((node) => {
        if (node.metadata?.status !== "loading" || hasResumableVideoTask(node)) return node;
        ...
        return { ...node, metadata: { ...node.metadata, status: "error" as const, errorDetails: i18n.t("canvas.generation.interrupted"), images, texts: node.metadata.texts?.map((text) => text.status === "loading" ? { ...text, status: "error" as const, errorDetails: i18n.t("canvas.generation.interrupted") } : text) } };
    });
}
