...
- [优化] GPT Image 模型选择改为先渠道后版本，明确分隔导航与选项，浏览渠道不再自动切换模型，收起时仅显示渠道与版本。

- [调整] 合并官方 v0.17.0 / v0.18.0 的分组引用、视频恢复、尺寸设置与本地代理，保留 SuperToken 授权、模型能力、批量生成和异步恢复。
...
- [修复] 修复生图工作台桌面版模型选择器因缺少更新回调而无法切换模型的问题。

+ [优化] SuperToken 异步任务在服务端未返回有效轮询间隔或遇到临时网络错误时，每轮随机等待 5-10 秒且不再指数退避，降低多任务并发压力。
+ [修复] SuperToken 的 Leonardo 参考素材下载遇到跨地区临时网络故障时有限重试，重试耗尽后画布会清理临时 URL 缓存并重新上传一次素材。
+ [调整] 本地源码 Docker Compose 部署的默认宿主机端口改为 10001。
+ [新增] 内置 SuperToken 图片与视频供应商，支持能力驱动的模型选项、原生异步任务、媒体上传、刷新恢复和本地结果保存。
+ [调整] SuperToken 视频模型只展示已接入且账号实际授权的 SKU，Veo 与旧版 Grok 15s 型号不进入可选列表。
+ [新增] SuperToken 新增 xAI Grok 图片与两个 720p 视频模型，支持文生图、按序多图编辑、文生视频、单图视频及双图参考，并沿用异步恢复、资源鉴权下载和本地持久化。
+ [修复] Grok 文生图与图生图从工作台或画布提交时保留所选画幅，不再误转为像素尺寸后被前端拒绝。
+ [调整] Grok 视频在模型选择器中标记为支持真人，并精简基础版与 Preview 的差异描述。
+ [调整] SuperToken 线路切换统一移到画布、生图工作台和视频工作台顶栏，并改用 Wi-Fi 状态图标入口。
+ [修复] SuperToken 任务只要返回有效 `progress` 就展示百分比，不再受 `progress_known` 标记限制。
+ [修复] 切换 SuperToken 视频模型时自动选择最低分辨率、最短时长、默认比例、参考模式和音轨设置，同时保留已添加的参考素材。
+ [修复] 未配置可用渠道时不再展示内置默认模型，模型选择器会直接引导到渠道配置，并允许 SuperToken 图片或视频能力独立启用。
+ [调整] 视频创作台与画布根据所选视频模型采用首尾帧、多图参考或混合素材的默认参考模式。
+ [优化] SuperToken 渠道将 Resource API Key 标记为必填项，缺失时明确提示并禁止保存。
+ [修复] SuperToken 视频任务为图片、视频和音频参考素材生成全局唯一别名，避免同名画布节点导致请求失败。
+ [修复] SuperToken 视频创作台按模型限制接收参考素材，生图工作台与画布按模型能力使用原生多图任务或单图任务，并完整恢复批量结果。
+ [新增] SuperToken 渠道支持通过 PKCE 授权自动创建或复用图片、视频和资源凭证，并立即同步可用模型与本地配置；授权弹窗、跨域兑换及取消/失败流程不会覆盖已有配置。
+ [修复] Docker 部署将 SuperToken 授权主站与地区媒体 API 节点分离配置，避免授权域名覆盖大陆加速和国际直连地址。
+ [优化] 生图、视频工作台和画布节点根据异步任务真实进度展示平滑显影动画，未知进度不再伪造百分比，刷新恢复后进度只前进不回退。
+ [修复] Docker Nginx 对缺失的 Vite 哈希资源返回 404，并禁止缓存入口 HTML 与运行期配置，避免发布后旧资源被回退页面和 CDN 缓存导致白屏。
+ [修复] Adobe 与 Leonardo 参考视频/音频按各模型上限允许最多 0.3 秒编码尾差，明显超限时在上传前给出明确提示，并由服务端返回结构化 400。
+ [新增] SuperToken 视频支持 Leonardo Seedance 2.5 的 480p/720p、4-30 秒及首尾帧/混合素材模式，并按接口返回动态展示 Adobe/Leonardo Seedance 新型号。
+ [新增] SuperToken 视频支持 MiniMax H3 的 768p、1440p、2160p 模型，并在分辨率和比例选择中展示 HD/2K/4K 档位与精确输出尺寸。
+ [优化] 生图工作台、视频工作台与画布生成输入统一限制为 2000 字符，并实时显示计数与到达上限的错误状态。
+ [修复] SuperToken 的 Leonardo Seedance 2.5 混合素材限制同步为 30 图、10 视频、10 音频，视频与音频分别允许合计 30.2 秒，首尾帧仍仅允许 1-2 张图片。
+ [修复] SuperToken 参考素材按完成响应的过期时间缓存临时 URL，过期前自动从本地素材重新上传，并在引用失效时清理缓存安全重试。
+ [修复] 画布生图失败节点保留并展示服务端错误码与原始消息，不再被“全部图片生成失败”通用文案覆盖。
+ [修复] 图片、视频、音频及自定义模型脚本请求统一保留服务端错误消息与错误码，避免 4xx/5xx 失败只能从浏览器请求中排查。
+ [修复] 未配置画布生图张数时默认生成 1 张，不再默认创建 3 张图片。
+ [优化] 视频模型选择器以创作类型、品牌和版本分层展示，并在同版本下清晰区分 Adobe 与 Leonardo 渠道；画布使用紧凑弹层，窄屏改用底部抽屉。
+ [新增] SuperToken 生图工作台、视频工作台和画布可全局同步切换加速或国际线路，收起时以低频状态水波保持紧凑，页面可见时每 2 分钟共享检测可用状态与响应延迟。
+ [优化] 生图模型选择器按 GPT Image 与 Gemini 家族导航，并以卡片区分 Azure、Adobe、第三方渠道及香蕉模型别名与核心能力。
+ [优化] 生图模型卡片推荐 Adobe 与小香蕉；首次配置优先 Adobe，Gemini 家族默认使用小香蕉，同时保留用户已有选择。
+ [优化] 已授权的 SuperToken 渠道每五分钟静默同步当前图片与视频模型，并同步校正 Canvas 专用 Token 白名单，无需为新增模型重复授权。
+ [修复] MiniMax H3 混合素材模式不再强制同时提供图片和参考音频，可按需使用图片、音频或纯提示词生成。
...
+ [调整] 合入官方视频生成请求精简；本分支继续保留火山方舟、Seedance、SuperToken 及视频/音频参考适配。
